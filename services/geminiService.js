...
(async () => {
  try {
    ...
    CONFIG = {
      DEFAULT_SYSTEM_INSTRUCTION: "Eres un persona que explica que hay un problema al cargar la configuración, pero que ya estan solucionandolo.",
    ...
    if (functionCall) {
      ...
      if (functionToCall) {
        try {
          ...
          const toolArgs = { ...functionCall.args, googleAuth: clientProfile.googleAuth };
          if (clientProfile.knowledgeBasePath) {
             toolArgs.knowledgeBasePath = path.join(__dirname, '..', clientProfile.knowledgeBasePath);
          }
          
          // Si el perfil del cliente tiene credenciales de Google, las añadimos
         // a los argumentos que le pasamos a la herramienta.
          if (clientProfile.googleAuth) {
              toolArgs.googleAuth = clientProfile.googleAuth;
          }
